# ======================================================
# Compute Mantel test between int / int freq and TRAIT matching matrix (by week)
# ======================================================
...
# ======================================================
# Read network data (trait matching by week)
...
mantel_network_trait <- function(garden_name, week, network_type) {
  ...
  trait_network <- df_filtered %>%
    pull(Trait_prob) %>%
    .[[1]]
  
  # Check for valid matrices
  if (is.null(interaction_network) | is.null(trait_network) |
      nrow(interaction_network) < 2 | nrow(trait_network) < 2 |
      anyNA(interaction_network) | anyNA(trait_network)) {
...
results_int_trait <- pmap_dfr(
  ...
  ~ mantel_network_trait(..1, ..2, "Interaction_network")
)
...
results_intFreq_trait <- pmap_dfr(
  ...
  ~ mantel_network_trait(..1, ..2, "Int_frequency_network")
)
...
results_intFreq_trait$Mantel_corr <- abs(results_intFreq_trait$Mantel_corr)

# Save (mirroring your abundance workflow)
saveRDS(results_intFreq_trait, "Data/Working_files/Mantel_trait_week_result.rds")
...
ggplot(combined_results, aes(x = Sampling_week,
  ...
  labs(x = "Sampling Week",
       ...
       title = "Mantel correlation by Garden, Week and Test Type (Trait matching)",
       fill = "Test Type") +
