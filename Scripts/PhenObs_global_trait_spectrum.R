...
#Replicate graph if possible and 
#see how to add species of the PollObs project

#Before prepare PollObs dataset to integrate it with the existing data
...
group_by(Species) %>%
  summarise(
    Average_Flowers_per_plant = mean(Flowers_per_plant, na.rm = TRUE),
    Average_Corolla_diameter_mean = mean(Corolla_diameter_mean, na.rm = TRUE),
    Average_Style_length = mean(Style_length, na.rm = TRUE),
    Average_Ovule_number = mean(Ovule_number, na.rm = TRUE),
    Average_Plant_height_mean_m = mean(Plant_height_mean_m, na.rm = TRUE)
  )
