...
ggplot(d1_tb, aes(d)) +
geom_histogram() +
geom_histogram(d2_tb, aes(d))



# Convert d1 and d2 to tibble format
d1_tb = as_tibble(d1)
d2_tb = as_tibble(d2)

# Plot histograms for both datasets
ggplot() +
  geom_histogram(data = d1_tb, aes(x = dprime), fill = "blue", alpha = 0.5, bins = 30) +
  geom_histogram(data = d2_tb, aes(x = dprime), fill = "red", alpha = 0.5, bins = 30) +
  labs(title = "Histogram of d1 and d2", x = "d values", y = "Frequency") +
  theme_minimal()
...
q <- colSums(n2) / sum(n2)
p_list <- apply(n2, 1, function(x) x / sum(x))
