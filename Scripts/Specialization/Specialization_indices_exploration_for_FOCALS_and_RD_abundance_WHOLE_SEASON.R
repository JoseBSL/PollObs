...
for (Botanical_garden_i in Botanical_garden_list) {
  ...
  plant_abundance_Garden_i <- interaction_data_season %>% ungroup() %>%
    filter(Botanical_garden == Botanical_garden_i) %>%
    dplyr::select(Plant, Total_floral_abundance) %>% unique() %>% ungroup()
  
  
  poll_abundance_Garden_i <-  interaction_data_season %>% ungroup() %>%
    filter(Botanical_garden == Botanical_garden_i) %>%
  ...
  all_results_specialization_plant_Garden_i <- 
    dfun(interaction_matrix_Garden_i, 
         abuns = poll_abundance_Garden_i$Total_pollinator_abundance)# plant_abundance_Garden_i$Plant_total_floral_abundance)
  ...
  all_results_specialization_poll_Garden_i <- 
    dfun(t(interaction_matrix_Garden_i), abuns = plant_abundance_Garden_i$Total_floral_abundance)
...
readr::write_csv(average_specialization,"Data/Working_files/average_d_prime_by_SEASON_corrected_by_ab.csv")
readr::write_csv(species_specialization_Garden,"Data/Working_files/species_d_prime_by_SEASON_corrected_by_ab.csv")
