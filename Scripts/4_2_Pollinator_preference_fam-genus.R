# ======================================================
# Pollinator preference exploration at plant family level
# and pollinator genus level
# ======================================================
...
int_data_clean = raw_data %>%
  ...
  select(Plant_family, Plant, Pollinator_genus, Pollinator_accepted_name, Botanical_garden) %>%
  drop_na()
...
phenobs_fam_vector = int_data_phenobs %>% distinct(Plant_family) %>% pull()

int_data_combined = bind_rows(
  int_data_phenobs,
  int_data_nonphenobs %>% filter(Plant_family %in% phenobs_fam_vector)
)
...
int_data_combined = int_data_combined %>%
  filter(Plant_family %in% families_to_keep)

# ======================================================
# Create preference table
pref.table = int_data_combined %>%
  count(Pollinator_genus, Plant_family) %>%
  pivot_wider(names_from = Plant_family, values_from = n, values_fill = 0) %>%
  column_to_rownames("Pollinator_genus")
...
obs_df = pref.table %>%
  as.data.frame() %>%
  rownames_to_column("Pollinator_genus") %>%
  pivot_longer(-Pollinator_genus, names_to = "Plant_family", values_to = "Observed")

# Convert null matrices into tidy tibble with SimID
sim_df = map2_dfr(n.mod, seq_along(n.mod), ~ as.data.frame(.x) %>%
                    rownames_to_column("Pollinator_genus") %>%
                    pivot_longer(-Pollinator_genus, names_to = "Plant_family", values_to = "Simulated") %>%
                    mutate(SimID = .y))

# Compute p-values in tidyverse
pval_df = sim_df %>%
  inner_join(obs_df, by = c("Pollinator_genus", "Plant_family")) %>%
  group_by(Pollinator_genus, Plant_family) %>%
...
ses_df = as.data.frame(SES) %>%
  rownames_to_column("Pollinator_genus") %>%
  pivot_longer(-Pollinator_genus, names_to = "Plant_family", values_to = "SES")

results = left_join(ses_df, pval_df, by = c("Pollinator_genus", "Plant_family")) %>%
  arrange(p_value)
