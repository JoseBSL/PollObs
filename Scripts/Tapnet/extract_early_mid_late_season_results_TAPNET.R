...
result_tapnet_partial_season <- NULL
...
for (garden_number in 1:length(gardens)) {
  ...
  for (season_number in 1:length(season)) {
    ...
    result <- try({
      ...
      aux_result_tapnet_partial_season <- tibble(
      ...
      result_tapnet_partial_season <- bind_rows(result_tapnet_partial_season, aux_result_tapnet_partial_season) 
      
    }, silent = TRUE)
    
    if (inherits(result, "try-error")) {
      message("Error on iteration ", garden_name, season_name, ". Skipping.")
    }
...
result_tapnet_partial_season_FINAL <- result_tapnet_partial_season %>%
  dplyr::select(Botanical_garden, Season, Variable, Value)
  

name_tapnet_partial_season <- "Data/Working_files/results_TAPNET_EARLY_MID_LATE_SEASON_traits.rds"
name_tapnet_partial_season_csv <- "Data/Working_files/results_TAPNET_EARLY_MID_LATE_SEASON_traits.csv"
saveRDS(result_tapnet_partial_season_FINAL, name_tapnet_partial_season)
readr::write_csv(result_tapnet_partial_season_FINAL, name_tapnet_partial_season_csv)
