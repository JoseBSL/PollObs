...
for (garden_number in 1:length(gardens)) {
  ...
  floral_ab_garden_full_season <- floral_ab_garden_full_season_raw %>%
    ...
    filter(Plant %in% rownames(garden_full_season_net[[1]]))
  ...
  poll_ab_garden_full_season <- poll_ab_garden_full_season_raw %>%
    ...
    filter(Pollinator %in% colnames(garden_full_season_net[[1]]))
  
  # Sanity check
  floral_ab_garden_full_season %>% filter(is.na(Plant))
  poll_ab_garden_full_season %>% filter(is.na(Pollinator))
  ...
  # save info
  name_tapnet_obj <- paste0("Data/Working_files/",garden_name,"_obj_TAPNET_full_season_NO_traits.rds")
