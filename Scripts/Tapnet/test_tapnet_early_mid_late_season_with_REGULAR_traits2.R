...
trait_axes_polls <- readRDS("Data/Trait_data/Processed/PollTraits_all.rds") %>% 
  dplyr::select(Pollinator_accepted_name, Proboscis_length_mean) %>% rename(Pollinator = Pollinator_accepted_name)
trait_axes_plants <- readRDS("Data/Working_files/plants_REAL_trait_phenobs.rds") %>% 
  dplyr::select(Species,Floral_tube_length) %>% rename(Plant = Species)
...
for (garden_number in 1:length(gardens)) {
  ...
  for (season_number in 1:length(season)) {
    ...
    tapnet_floral_trait_axes_partial_season <- floral_trait_axes_partial_season %>%
      dplyr::select(Floral_tube_length)
    ...
    tapnet_poll_trait_axes_partial_season <- poll_trait_axes_partial_season %>%
      dplyr::select(Proboscis_length_mean)
    ...
    # save info
    name_tapnet_obj <- paste0("Data/Working_files/",garden_name,"_obj_TAPNET_",season_name,"_season_WITH_REGULAR_traits2.rds")
    name_fit <- paste0("Data/Working_files/",garden_name,"_fit_TAPNET_",season_name,"_season_WITH_REGULAR_traits2.rds")
    name_gof <- paste0("Data/Working_files/",garden_name,"_gof_TAPNET_",season_name,"_season_WITH_REGULAR_traits2.rds")
