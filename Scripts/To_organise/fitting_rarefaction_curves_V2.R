...
nlm_Halle_focal <- nls(Mean_cumulative_spp ~ a * (1 - exp(-Mean_cumulative_time / b)), start = list(a = 1, b = 100),
...
Halle_focal_points$Predicted_nlm <- predict(nlm_Halle_focal, 
                                            newdata = Halle_focal_points, type = "response")
...
nlm_Halle_Random_census <- 
 nls(Mean_cumulative_spp ~ a * (1 - exp(-Mean_cumulative_time / b)), start = list(a = 1, b = 100),
     data = Halle_Random_census_points)
summary(nlm_Halle_Random_census)


# Add predicted values (on response scale)
Halle_Random_census_points$Predicted_nlm <- predict(nlm_Halle_Random_census, 
                                                    newdata = Halle_Random_census_points, type = "response")
...
Mean_cumulative_time <- seq(0,6000,by=10)
fits_Halle <- data.frame(Mean_cumulative_time = Mean_cumulative_time,
                         Richness_Random = coef_Halle_Random["a"] * (1 - exp(-Mean_cumulative_time/coef_Halle_Random["b"])),
                         Richness_Focal = coef_Halle_Focal["a"] * (1 - exp(-Mean_cumulative_time/coef_Halle_Focal["b"])))

plot_Halle <- ggplot(data = fits_Halle, aes(x = Mean_cumulative_time, y = Richness_Random))+
  geom_path(color = "red")+
  geom_path(data = fits_Halle, aes(x = Mean_cumulative_time, y = Richness_Focal), color = "blue")+
  ...
  geom_point(data = Halle_Random_census_points, aes(x = Mean_cumulative_time, y = Mean_cumulative_spp,
                                                    color = Plant_sampled))+
  geom_point(data = Halle_Focal_points, aes(x = Mean_cumulative_time, y = Mean_cumulative_spp,
                                            color = Plant_sampled))+
  labs(title = "Halle", x = "Mean_cumulative_time", y = "Pollinator richness")+
  annotate("text", x = max(fits_Halle$Mean_cumulative_time), y = max(fits_Halle$Richness_Random),
           label = "Random", color = "red", hjust = 1) +
  annotate("text", x = max(fits_Halle$Mean_cumulative_time), y = max(fits_Halle$Richness_Focal),
...
nlm_Jena_Focal <- nls(Mean_cumulative_spp ~ a * (1 - exp(-Mean_cumulative_time / b)), start = list(a = 1, b = 100),
...
# Add predicted values
Jena_focal_points$Predicted_nlm <- predict(nlm_Jena_Focal, 
                                            newdata = Jena_focal_points, type = "response")
...
# Add predicted values (on response scale)
Jena_Random_census_points$Predicted_nlm <- predict(nlm_Jena_Random_census, 
                                                    newdata = Jena_Random_census_points, type = "response")
...
Mean_cumulative_time <- seq(0,6000,by=10)
fits_Jena <- data.frame(Mean_cumulative_time = Mean_cumulative_time,
                         Richness_Random = coef_Jena_Random["a"] * (1 - exp(-Mean_cumulative_time/coef_Jena_Random["b"])),
                         Richness_Focal = coef_Jena_Focal["a"] * (1 - exp(-Mean_cumulative_time/coef_Jena_Focal["b"])))

plot_Jena <- ggplot(data = fits_Jena, aes(x = Mean_cumulative_time, y = Richness_Random))+
  geom_path(color = "red")+
  geom_path(data = fits_Jena, aes(x = Mean_cumulative_time, y = Richness_Focal), color = "blue")+
  ...
  geom_point(data = Jena_Random_census_points, aes(x = Mean_cumulative_time, y = Mean_cumulative_spp,
                                                    color = Plant_sampled))+
  geom_point(data = Jena_Focal_points, aes(x = Mean_cumulative_time, y = Mean_cumulative_spp,
                                            color = Plant_sampled))+
  labs(title = "Jena", x = "Mean_cumulative_time", y = "Pollinator richness")+
  annotate("text", x = max(fits_Jena$Mean_cumulative_time), y = max(fits_Jena$Richness_Random),
           label = "Random", color = "red", hjust = 1) +
  annotate("text", x = max(fits_Jena$Mean_cumulative_time), y = max(fits_Jena$Richness_Focal),
           label = "Focal", color = "blue", hjust = 1) +
  theme_bw()
...
nlm_Leipzig_Focal <- nls(Mean_cumulative_spp ~ a * (1 - exp(-Mean_cumulative_time / b)), start = list(a = 1, b = 100),
...
# Add predicted values
Leipzig_focal_points$Predicted_nlm <- predict(nlm_Leipzig_Focal, 
                                           newdata = Leipzig_focal_points, type = "response")
...
# Add predicted values (on response scale)
Leipzig_Random_census_points$Predicted_nlm <- predict(nlm_Leipzig_Random_census, 
                                                   newdata = Leipzig_Random_census_points, type = "response")
...
Mean_cumulative_time <- seq(0,6000,by=10)
fits_Leipzig <- data.frame(Mean_cumulative_time = Mean_cumulative_time,
                        Richness_Random = coef_Leipzig_Random["a"] * (1 - exp(-Mean_cumulative_time/coef_Leipzig_Random["b"])),
                        Richness_Focal = coef_Leipzig_Focal["a"] * (1 - exp(-Mean_cumulative_time/coef_Leipzig_Focal["b"])))

plot_Leipzig <- ggplot(data = fits_Leipzig, aes(x = Mean_cumulative_time, y = Richness_Random))+
  geom_path(color = "red")+
  geom_path(data = fits_Leipzig, aes(x = Mean_cumulative_time, y = Richness_Focal), color = "blue")+
  ...
  geom_point(data = Leipzig_Random_census_points, aes(x = Mean_cumulative_time, y = Mean_cumulative_spp,
                                                   color = Plant_sampled))+
  geom_point(data = Leipzig_Focal_points, aes(x = Mean_cumulative_time, y = Mean_cumulative_spp,
                                           color = Plant_sampled))+
  labs(title = "Leipzig", x = "Mean_cumulative_time", y = "Pollinator richness")+
  annotate("text", x = max(fits_Leipzig$Mean_cumulative_time), y = max(fits_Leipzig$Richness_Random),
           label = "Random", color = "red", hjust = 1) +
  annotate("text", x = max(fits_Leipzig$Mean_cumulative_time), y = max(fits_Leipzig$Richness_Focal),
           label = "Focal", color = "blue", hjust = 1) +
  theme_bw()
