...
int_data_clean = raw_data %>%
  ...
  select(Plant_family, Plant, Pollinator_family, Pollinator_accepted_name, Botanical_garden) %>%
  drop_na()
...
results = left_join(ses_df, pval_df, by = c("Pollinator_family", "Plant_family")) %>%
  arrange(p_value)
