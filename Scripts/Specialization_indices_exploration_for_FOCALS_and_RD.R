...
species_i = "Apis mellifera"
ggplot(species_specialization_Garden_Week %>% filter(sp_name == species_i), 
       aes(x=Week,y=dprime, color = Botanical_garden))+
  geom_point(size=3)+
  geom_line(linewidth=1.5)+
  ylim(0,1)+
  labs(title = species_i,
    x="Week number", y=  "Average species level\nspecialization index", color = NULL)+
  theme_bw()+
  theme(legend.position = "bottom", legend.title = element_blank())+
  theme(legend.text = element_text(size = 18),
        axis.text=element_text(size=16),
