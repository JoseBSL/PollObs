...
#DO NOT RUN, now use only 3_4 to adjust values
...
#spp_present_phen = spp_order_raw[1:(match("Dolichovespula saxonica", spp_order_raw) - 1)]
#
##Select only records for species with missing phenology 
#occurrences1 = occurrences %>% 
#filter(species %in% spp_missing_phen) %>% 
...
#  distinct() %>% 
#  filter(!Species %in% spp_present_phen)
#
##To match cols with interaction data
#oc = oc %>% 
#  mutate(PollObs = "No") 
#
##Bind rows
#oc = bind_rows(oc, doy_yes)
#
