#REPRODUCTIVE TRAIT SPECTRUM of PHENOBS PLANTS
#In this code we only include the traits of Lanuza 2023
library(readr)
library(dplyr)
library(phytools) 
library(ape) 
...
phenobs_data = left_join(selfing, morphometrics)

colnames(phenobs_data)
phenobs_data = phenobs_data %>% 
...
plant_spp = rownames(phenobs_data)
...
matched_gbif_plants1$Species_all == rownames(phenobs_data)
pollobs_data1 = bind_cols(phenobs_data, matched_gbif_plants1)

#Check colnames
colnames(pollobs_data1)
pollobs_data1 = pollobs_data1 %>%
  left_join(phenobs_metrics, by = "Species_all") 
colnames(pollobs_data1)

#Check correlation
cor.test(pollobs_data1$Mean_interaction_frequency,
         pollobs_data1$Mean_normalised_degree)


# Define the columns to transform
cols_to_transform = c("Autonomous_selfing_level_fruit_set", "Flowers_per_plant",
                       "Corolla_diameter_mean", "Style_length", "Ovule_number", 
                       "Plant_height_mean_m", "Mean_interaction_frequency",
                      "Mean_normalised_degree")

# Conduct log transformation and scaling using mutate across selected columns
final_d = pollobs_data1 %>%
...
rownames(final_d) = pollobs_data1$Species_all
...
local_phenobs_pca <- function(PC, x = "PC1", y = "PC2") {
  ...
  rownames(PC$L) <- c("S", "FN", "FS", "SL", "ON", "PH", "IF", "ND")
  PCAloadings <- data.frame(Variables = rownames(PC$L), PC$L)
  
  # Position labels at the end of segments
  label_positions <- -datapc[, c("v1", "v2")]
  
  plot <- plot + annotate("text", x = label_positions[[1]]*c(1.1,1.2,1.1,1.18,1,1.15,1.25,1), y = label_positions[[2]]*c(2,1.35,1,0.9,1,1.5,2.5,1), 
...
#This is a way to explore it but
#maybe just showing the corplot may work
