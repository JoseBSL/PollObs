...
# Crear data frame para 4 polinizadores
pollinadores <- data.frame(
  dia = rep(dias, 4),
  densidad = 10 * c(
    dnorm(dias, mean=50,  sd=20),   
    dnorm(dias, mean=100, sd=30),   
    dnorm(dias, mean=140, sd=30),   
    dnorm(dias, mean=220, sd=20)    
  ),
  especie = factor(rep(c("a", "b", "c", "d"), each=length(dias))),
  tipo = "Insect"
)

# Crear data frame para 4 plantas
plantas <- data.frame(
  dia = rep(dias, 4),
  densidad = 10 * c(
    dnorm(dias, mean=60,  sd=25),   
    dnorm(dias, mean=110, sd=25),   
    dnorm(dias, mean=150, sd=25),   
...
ggplot(fenologia, aes(x=dia, y=densidad, fill=especie, color=especie)) +
  ...
  facet_wrap(~ tipo, scales = "free_y", ncol = 1) +  # stacked vertically
  theme_bw() +
  theme(
