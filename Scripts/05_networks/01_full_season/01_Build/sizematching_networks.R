# =========================================================
# Trait probability matrices per garden
# - Same row/col alignment as Interaction_network
# - Filtered to garden species sets
# =========================================================
...
#     aligned to each garden interaction network
# -----------------------------
build_trait_matrix <- function(garden_name, trait_mat, normalize = FALSE) {
...
trait_prob_matrices_by_garden <- net_by_garden %>%
  mutate(
    Trait_prob = map(Botanical_garden, ~build_trait_matrix(.x, T_gauss_mat, normalize = FALSE)),
    Trait_prob_rowNorm = map(Botanical_garden, ~build_trait_matrix(.x, T_gauss_mat, normalize = TRUE))
  )
...
# -----------------------------
saveRDS(
  trait_prob_matrices_by_garden,
  "Data/Working_files/trait_probability_networks_only_phenobs.rds"
)

# Object contains:
# - Botanical_garden
# - Interaction_network (existing)
# - Trait_prob (aligned matrix)
# - Trait_prob_rowNorm (aligned + row-normalized matrix)
