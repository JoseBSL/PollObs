...
interaction_week = raw_week %>%
  ...
  group_by(Botanical_garden, Plant, Pollinator, Week, Sampling_type) %>%
  summarise(
...
plant_week_effort = raw_week %>%
  ...
  group_by(Botanical_garden, Plant, Week, Sampling_type) %>%
  summarise(
...
interaction_data = interaction_week %>%
  left_join(plant_week_effort, by = c("Botanical_garden", "Plant", "Week", "Sampling_type")) %>%
  mutate(
...
weekly_data = interaction_data %>%
  ...
  left_join(floral_abundance_week,   by = c("Botanical_garden", "Plant", "Week", "Sampling_type")) %>%
  left_join(poll_abundance_week,     by = c("Botanical_garden", "Pollinator", "Week", "Sampling_type"))
...
# Save data
saveRDS(weekly_data, "Data/Working_files/weekly_data_for_rewiring.rds")
################################################################################
