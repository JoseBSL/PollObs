...
#Load Halle data
plant_phen_halle = readRDS("Data/Phenology_data/clean_plant_phenobs_halle.rds")

spp_name_halle = plant_phen_halle %>% 
distinct(Species) %>%
pull()

unique_species_sequence = plant_phen_halle %>% 
distinct(Species) %>% 
group_by(Species) %>% 
tidyr::crossing(Doy = 1:365)

#Bind both datasets
plant_phen = left_join(unique_species_sequence, plant_phen_halle)
...
mutate(k_value = case_when(
            Species == "Viola odorata" ~ 5,
            Species == "Lamium album" ~ 5,
            Species == "Polygonum bistorta" ~ 5,
            Species == "Chelidonium majus" ~ 5,
            Species == "Gratiola officinalis" ~ 5,
            Species == "Helianthemum nummularium" ~ 5,
            Species == "Hypericum olympicum" ~ 5,
            Species == "Leucanthemum vulgare" ~ 5,
            Species == "Hypericum perforatum" ~ 5,
            Species == "Platycodon grandiflorus" ~ 5,
            Species == "Salvia officinalis" ~ 5,
            Species == "Hemerocallis fulva" ~ 5,
            Species == "Clematis integrifolia" ~ 6,
            Species == "Clematis recta" ~ 6,
            Species == "Dianthus carthusianorum" ~ 5,
            Species == "Saponaria officinalis" ~ 5,
            Species == "Fuchsia magellanica" ~ 6,

            TRUE ~ k_value))


#Define the function
predict_flowering_intensity <- function(spp_name_halle, plant_phen) {
  # Filter data for the specified species
  sp_data <- plant_phen %>% filter(Species == spp_name_halle)
  ...
  sp_data <- sp_data %>% 
    ...
    mutate(Flowering_intensity = if_else(Doy > max(sp_data$Doy[sp_data$Flowers_opening == "y"], na.rm = TRUE),
     ...
     # Add 0!
  if (spp_name_halle == "Viola odorata") {
    sp_data <- sp_data %>%
      mutate(Flowering_intensity = if_else(Doy > 110 & Doy < 280, 0, Flowering_intensity))
  }
  
       # Exclude 0's 
  if (spp_name_halle == "Geum rivale") {
    sp_data <- sp_data %>%
      mutate(Flowering_intensity = if_else(Doy > 30 & Doy < 200 & Flowering_intensity == 0, NA, Flowering_intensity))
  }
  
         # Exclude 0's 
  if (spp_name_halle == "Silene viscaria") {
    sp_data <- sp_data %>%
      mutate(Flowering_intensity = if_else(Doy > 50 & Doy < 200 & Flowering_intensity == 0, NA, Flowering_intensity))
  }
  
           # Exclude 0's 
  if (spp_name_halle == "Fragaria vesca") {
    sp_data <- sp_data %>%
      mutate(Flowering_intensity = if_else(Doy > 50 & Doy < 200 & Flowering_intensity == 0, NA, Flowering_intensity))
  }
  
             # Exclude 0's 
  if (spp_name_halle == "Lamium album") {
    sp_data <- sp_data %>%
      mutate(Flowering_intensity = if_else(Doy > 90 & Doy < 300 & Flowering_intensity == 0, NA, Flowering_intensity))
  }
  
       # Add 0's 
  if (spp_name_halle == "Polygonum bistorta") {
    sp_data <- sp_data %>%
      mutate(Flowering_intensity = if_else(Doy > 170 & Doy < 200, 0, Flowering_intensity))
  }
  
         # Exclude 0's 
   if (spp_name_halle == "Chelidonium majus") {
    sp_data <- sp_data %>%
      mutate(Flowering_intensity = if_else(Doy > 110 & Doy < 300 & Flowering_intensity == 0, NA, Flowering_intensity))
  }
  
           # Exclude 0's 
   if (spp_name_halle == "Salvia officinalis") {
    sp_data <- sp_data %>%
      mutate(Flowering_intensity = if_else(Doy > 130 & Doy < 300 & Flowering_intensity == 0, NA, Flowering_intensity))
  }
  
            # Exclude 0's 
   if (spp_name_halle == "Solanum dulcamara") {
    sp_data <- sp_data %>%
      mutate(Flowering_intensity = if_else(Doy > 100 & Doy < 270 & Flowering_intensity == 0, NA, Flowering_intensity))
  }
  
  
            # Exclude 0's 
   if (spp_name_halle == "Gratiola officinalis") {
    sp_data <- sp_data %>%
      mutate(Flowering_intensity = if_else(Doy > 100 & Doy < 300 & Flowering_intensity == 0, NA, Flowering_intensity))
   }
  
              # Exclude 0's 
   if (spp_name_halle == "Leucanthemum vulgare") {
    sp_data <- sp_data %>%
      mutate(Flowering_intensity = if_else(Doy > 100 & Doy < 300 & Flowering_intensity == 0, NA, Flowering_intensity))
  }
  ...
  plot <- ggplot(unique_dates, aes(x = Doy, y = Probability)) +
    ...
    labs(title = paste("Predicted Probabilities for", spp_name_halle, "Over Day of Year (Doy)"),
         x = "Day of Year (Doy)",
...
# Assuming plant_phen and spp_name_halle are already defined
predictions_list <- list()

for (species in spp_name_halle) {
  predictions <- predict_flowering_intensity(spp_name_halle = species, plant_phen = plant_phen)
...
sp_tibble = tibble(Species = spp_name_halle)
...
for (i in 1:length(all_predictions1$Species)) {
  ...
  if (!is.null(plot)) {
    ...
    plot_filename <- paste0("Data/Plant_phenology/Halle/", i, "_", stringr::str_replace(species, " ","_"), ".png")
...
#Save data
saveRDS(all_binded2, "Data/Working_files/halle_plant_phenology_predictions.rds")
