...
```{r ,echo=FALSE, message=FALSE, cache=FALSE, warning=FALSE, fig.height=8,fig.width=8,out.width="105%", fig.cap= "\\textbf{Figure 4}. Heatmap of standardized effect sizes (SES) comparing observed plant-pollinator interactions at family level with expectations from a spatiotemporally constrained null model. Purple indicates avoidance and orange indicates association relative to the null expectation. Asterisks denote significant deviations (\\textit{p} < 0.05). Dark grey cells represent plant-pollinator family pairs that never co-occurred within the same garden and sampling week and were therefore impossible given the sampling structure."}
